...
Page({
  ...
  data: {
    ...
    userInfo: {},
    openid: '',
    animation: 'animated bounceIn infinite',
    hasUserInfo: false,
  },
  ...
  onLoad: function (options) {
    wx.cloud.callFunction({ //Get openid via cloud function
      name: 'login',
      data: {},
      success: res => {
        wx.setStorage({
  ...
  onGetUserInfo: function (e) {
    ...
    if (!this.data.logged && this.data.hasUserInfo) { //Log on Successfully
      this.setData({
        logged: true
      })
      this.register();
    } else {
      console.log("Did not go into if chunk");
    }
  },
  register: function () {
    ...
    db.collection('User').where({
      ...
      success: res => { //When it's not registerd, it'll still run this funcction
        console.log(res.data.length);
        if (res.data.length == 0) { //Not Registered
